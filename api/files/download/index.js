...
async function handleDownload(req, res) {
  try {
    ...
    // Large files: use Git Blobs API with SHA
    if (!base64 && data.sha) {
      ...
      if (!blobRes.ok) {
        return res.status(blobRes.status).json({ error: `Blob API error: ${blobRes.status}` });
      }
      const blobData = await blobRes.json();
      if (blobData.encoding !== 'base64' || !blobData.content) {
        return res.status(500).json({ error: 'Cannot read file binary content' });
      }
